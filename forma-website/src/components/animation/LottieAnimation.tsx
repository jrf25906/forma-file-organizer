 ...
 *
 * A reusable Lottie animation component with dynamic JSON loading from
 * /public/animations/. Supports autoplay, loop, hover-triggered playback,
 * and viewport-triggered playback via IntersectionObserver.
 * Includes error handling with silent production fallback.
 *
 * @example
 * ```tsx
 * // Basic usage
 ...
 *
 * // Play on hover
 * <LottieAnimation animationPath="icon.json" playOnHover loop={false} />
 *
 * // Play when scrolled into view
 * <LottieAnimation animationPath="scroll.json" playOnView viewThreshold={0.3} />
 * ```
 */
export function LottieAnimation({
  ...
  const shouldPlay =
    shouldAutoplay || (playOnHover && isHovering) || (playOnView && isInView);
