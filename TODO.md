...
- [x] Constrain first-run folder setup to Desktop and Downloads with lightweight cleanup templates, leaving Documents, Pictures, and Music opt-in from Settings.
...
- [x] A3 — Added explicit `selectedTests` to the default, Unit, Performance, and UI `xctestplan` files; un-orphaned current Unit-plan suites; added `Scripts/verify_test_plan_membership.py` plus CI coverage to diff discovered test suites against plan membership.
...
- [ ] B4 — Decide fate of `DestinationPredictionService` drift detection and confidence-separation acceptance gate: Phase 0 evaluator repairs landed (shared Core ML probability parsing when exposed, off-main train/compile/load/evaluation, explicit label-only rejection without invented confidence, confidence-separation rejection notes, bounded sample-before-cap dataset split, POSIX UTC model versions); Phase 1 offline backtest harness landed and the first local run produced 116 resolved records (92 train / 24 holdout), below the 500-record gate, with pattern-only Top-1 at 2/24 and ML Top-1 at 0/24 because Core ML prediction probability output remained unavailable. Phase 2 live shadow measurement is blocked until enough resolved history exists to rerun the gate or the audit pivots to removing the ML branch.
