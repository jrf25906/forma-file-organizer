...
### Fixed
- Startup now removes historical exact duplicate Smart Rules while preserving one keeper per behavior, per-folder template application avoids SwiftData relationship auto-insertion before duplicate checks, placeholder rule destinations remain readable when bookmark data is absent, and bulk destination edits route rule creation through `RuleService` instead of directly inserting duplicate extension rules.
- Review-mode dashboard reads now reuse a cached review-pass projection instead of recomputing the same chunk, sections, ready counts, category summaries, and action-bar state on every SwiftUI binding read; live single-file organize paths also avoid a duplicate filter refresh after scan-list mutations, reducing UI stalls in large review queues.
...
- Workflow audit abandonment now stays visible: when `WorkflowRunner` stops after an upstream file failure, every remaining planned file gets `.skipped` execution-step audit rows and a skipped `WorkflowFileActionRecord` with the explicit `abandonedAfterUpstreamFailure` reason.
- App Intent entry points now fail closed until `FormaActions` is fully configured, `ToggleAutomationIntent` asks for explicit current-to-next confirmation before changing automation mode, and selected-item intents reject URLs that lack security-scoped bookmark access and are outside existing granted folder scopes.
