...
- Workflow Engine v2 metadata-step planning foundation:
  ...
  - `WorkflowInvocationContext.supportsProjectMetadataSteps`, `WorkflowInvocationContext.workflowProjectLabel`, `WorkflowTemplateCatalog`, and `WorkflowPlanner` now let `Project Drop Zone` project-space and project-policy runs plan `rename -> tag -> projectAssociation -> workflowStatus -> move -> log`, while keeping non-project invocations on the existing path.
