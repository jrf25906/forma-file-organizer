# Forma Documentation

Welcome to the Forma File Organizing App documentation.

**Status:** Current  
**Last Updated:** 2026-02-06  
**Audience:** Users | Developers | Designers | Security

## How to Navigate

- Start with the full map in [Docs/INDEX.md](INDEX.md).
- Each folder has a README that lists every doc in that area.
- Documentation conventions live in [Docs/DOCUMENTATION_STYLE.md](DOCUMENTATION_STYLE.md).

## Documentation Map

| Area | Index |
|------|-------|
| Getting Started | [Docs/Getting-Started/README.md](Getting-Started/README.md) |
| Architecture | [Docs/Architecture/README.md](Architecture/README.md) |
| Features | [Docs/Features/README.md](Features/README.md) |
| Design | [Docs/Design/README.md](Design/README.md) |
| Development | [Docs/Development/README.md](Development/README.md) |
| Testing | [Docs/Testing/README.md](Testing/README.md) |
| Security | [Docs/Security/README.md](Security/README.md) |
| Research | [Docs/Research/README.md](Research/README.md) |
| Roadmap | [Docs/Roadmap/README.md](Roadmap/README.md) |
| Legal | [Docs/Legal/README.md](Legal/README.md) |
| API Reference | [Docs/API-Reference/README.md](API-Reference/README.md) |
| Archive | [Docs/Archive/README.md](Archive/README.md) |

## Quick Links

| Topic | Document |
|-------|----------|
| Install and permissions | [Getting Started](Getting-Started/SETUP.md) |
| User guide | [User Guide](Getting-Started/USER-GUIDE.md) |
| Developer onboarding | [Developer Onboarding](Development/DEVELOPER-ONBOARDING.md) |
| Architecture overview | [Architecture](Architecture/ARCHITECTURE.md) |
| Design system | [Design System](Design/DesignSystem.md) |
| Security docs | [Security README](Security/README.md) |
| API reference | [API Reference](API-Reference/API_REFERENCE.md) |

## Documentation Principles

1. **Single Source of Truth** - Each topic has one authoritative document
2. **Categorized by Purpose** - Docs organized by how they're used
3. **Clear Entry Points** - README files guide you to the right docs
4. **Archive, Don't Delete** - Historical docs preserved for reference
