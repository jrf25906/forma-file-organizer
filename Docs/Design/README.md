# Design Documentation

> Visual design, brand identity, UX specifications, and design system for Forma.

**Status:** Current  
**Last Updated:** 2026-01-06  
**Audience:** Designers | Developers | Product

---

## Quick Navigation

| Document | Description | Priority |
|----------|-------------|----------|
| [BRAND-POSITIONING.md](BRAND-POSITIONING.md) | **Strategic source of truth** - Messaging, positioning, voice | Core |
| [DesignSystem.md](DesignSystem.md) | Design tokens, colors, typography | Core |
| [UI-GUIDELINES.md](UI-GUIDELINES.md) | UI implementation patterns | Core |
| [Forma-Brand-Guidelines.md](Forma-Brand-Guidelines.md) | Comprehensive brand implementation | Reference |

---

## Documents

### Design System
- **[DesignSystem.md](DesignSystem.md)** - Complete design token system: colors, typography, spacing, effects
- **[UI-GUIDELINES.md](UI-GUIDELINES.md)** - UI implementation patterns and conventions
- **[microanimations.md](microanimations.md)** - Animation patterns, timing, and easing

### Brand & Identity
- **[BRAND-POSITIONING.md](BRAND-POSITIONING.md)** - Strategic source of truth for messaging (all docs defer to this)
- **[BRAND-OVERVIEW.md](BRAND-OVERVIEW.md)** - Quick reference for developers/designers
- **[Forma-Brand-Guidelines.md](Forma-Brand-Guidelines.md)** - Comprehensive implementation guide
- **[BRAND_ASSETS.md](BRAND_ASSETS.md)** - Brand asset inventory
- **[LOGO_DESIGN_BRIEF.md](LOGO_DESIGN_BRIEF.md)** - Logo specifications
- **[FORMA-BRAND-TODO.md](FORMA-BRAND-TODO.md)** - Brand tasks

### User Experience
- **[Forma-Onboarding-Flow.md](Forma-Onboarding-Flow.md)** - Multi-step onboarding flow design
- **[Forma-Design-Doc.md](Forma-Design-Doc.md)** - Core design documentation
- **[forma-ui-spec.md](forma-ui-spec.md)** - UI specifications
- **[Forma-Empty-States.md](Forma-Empty-States.md)** - Empty state designs
- **[Forma-Keyboard-Shortcuts.md](Forma-Keyboard-Shortcuts.md)** - Keyboard shortcut mappings

### Interaction Design
- **[Forma-Confidence-Scoring.md](Forma-Confidence-Scoring.md)** - Confidence scoring UI
- **[Forma-Conflict-Resolution.md](Forma-Conflict-Resolution.md)** - Conflict resolution UX
- **[Forma-Rule-Library.md](Forma-Rule-Library.md)** - Rule library interface

### Technical Design
- **[Forma-SwiftData-Audit.md](Forma-SwiftData-Audit.md)** - Data model review

### Reference & Standards
- **[APPLE-DESIGN-AWARD-STANDARDS.md](APPLE-DESIGN-AWARD-STANDARDS.md)** - Apple Design Award criteria
- **[DESIGN-REVIEW-PROMPT.md](DESIGN-REVIEW-PROMPT.md)** - Design review guidelines

### Release
- **[Forma-App-Store-Description.md](Forma-App-Store-Description.md)** - App Store copy
- **[Forma-Rollout-Plan.md](Forma-Rollout-Plan.md)** - Release plan
- **[Forma-Test-Scenarios.md](Forma-Test-Scenarios.md)** - Test scenarios

---

## Design Tokens Overview

Design tokens live in [DesignSystem.md](DesignSystem.md) and are the source of truth for colors, typography, spacing, and effects.

---

## Related Documentation

- [Component Architecture](../Architecture/ComponentArchitecture.md) - UI component catalog
- [Feature Specifications](../Features/README.md) - Feature documentation
- [Personality System](../Features/PersonalitySystem.md) - Personality quiz UX
- [Documentation Index](../INDEX.md) - Master navigation
