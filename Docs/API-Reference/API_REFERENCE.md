...
- `WorkflowRunner` now records abandoned planned files after an upstream file failure as `.skipped` execution-step audit rows plus a skipped `WorkflowFileActionRecord` with the explicit `abandonedAfterUpstreamFailure` reason.
- `FormaIntentRuntime` now centralizes App Intent preflight checks: every `FormaAppIntents.perform()` path fails before dispatch unless `FormaActions.shared.isFullyConfigured`, selected-item intents require bookmark-backed access or an existing granted folder scope, and `ToggleAutomationIntent` requests explicit current-to-next confirmation before toggling automation.
- `DestinationPredictionBacktestRunner` is a test-target B4 Phase 1 harness that loads historical `ActivityItem` rows, extracts records through `LearningService.makeTrainingRecords(from:)`, chronologically splits 80/20, scores pattern-only and ML-only Top-1 accuracy overall plus category/source buckets, writes CSV/JSON evidence, and keeps the Phase 2 gate blocked unless at least 500 resolved records exist and ML beats pattern-only by 10 percentage points.
