...
- **App Intent entry points fail closed**: `FormaAppIntents` now gate `perform()` on `FormaActions.shared.isFullyConfigured`, `ToggleAutomationIntent` requires an explicit current-to-next `requestConfirmation` before mutating automation mode, and selected-item intents reject URLs unless the incoming `IntentFile` can provide security-scoped bookmark access or already lives inside a granted folder scope.
- **Destination prediction backtest gate recorded**: an integration-gated B4 Phase 1 harness now scores pattern-only versus ML-only destination prediction against historical `ActivityItem` training records, writes CSV/JSON evidence, and records the first local run as blocked: 116 resolved records, below the 500-record gate, with pattern-only Top-1 at 2/24 and ML Top-1 at 0/24.
